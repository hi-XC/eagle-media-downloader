...
This plugin imports images and videos from user-submitted public Instagram post links into the current Eagle library. This release accepts only HTTPS `instagram.com` URLs (including valid subdomains) with an exact `/p/<shortcode>` path. It does not access private or login-required content.

本插件将用户主动提交的 Instagram 公开帖子中的图片和视频导入当前 Eagle 素材库。当前版本只接受 HTTPS `instagram.com`（含合法子域名）的 `/p/<shortcode>` 路径，不访问私密或需要登录的内容。
...
- Source URLs are restricted before queueing, retrying, and invoking yt-dlp. HTTP redirects are checked one hop at a time and must remain within the supported Instagram post URL policy.
- Instagram media URLs are restricted to HTTPS Instagram/Facebook media delivery domains before Eagle imports them.
...
5. Start the download and confirm that each accessible image or video is imported as a separate Eagle item.
